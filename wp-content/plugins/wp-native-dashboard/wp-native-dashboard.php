...
if (!function_exists('plugins_url')) {
	...
	{
		return WP_PLUGIN_URL . '/' . dirname(plugin_basename($plugin));
	}
}
...
class wp_native_dashboard
{
	...
	function on_callback_help_overview()
	{
		?>
		<p>
			<strong>WP Native Dashboard </strong> -
			<em>"<?php _e('... use your backend with your prefered language', "wp-native-dashboard"); ?>"</em>
		</p>
		<p>
		...
		<p style="margin-top: 50px;padding-top:10px; border-top: solid 1px #ccc;">
			<small class="alignright" style="position:relative; margin-top: -30px; color: #aaa;">&copy; 2008 - 2013 by
				Heiko Rabe
			</small>
			...
			<a class="alignright" href="http://wordpress.org/extend/plugins/codestyling-localization/"
			   target="_blank"><?php _e('Translate WordPress, Themes and Plugins into your Language', "wp-native-dashboard"); ?></a>
		</p>
	<?php
	}
	...
	function on_print_metabox_content_acl($data)
	{
		?>
		<p>
			<input id="enable_login_selector" type="checkbox" value="1"
			       name="enable_login_selector"<?php if ($this->options->enable_login_selector) echo ' checked="checked"'; ?> />
			<?php _e('extend the <em>WordPress Logon Screen</em> to choose a language too.', "wp-native-dashboard"); ?>
		</p>
		<p style="margin-left: 16px;">
			<input id="enable_login_custom" type="checkbox" value="1"
			       name="enable_login_custom"<?php if ($this->options->enable_login_custom) echo ' checked="checked"'; ?> />
			<?php _e('enable template tag usage of login selector (see help screen).', "wp-native-dashboard"); ?>
		</p>
		<p>
			<input id="enable_profile_extension" type="checkbox" value="1"
			       name="enable_profile_extension"<?php if ($this->options->enable_profile_extension) echo ' checked="checked"'; ?> />
		...
		<p>
			<input id="enable_language_switcher" type="checkbox" value="1"
			       name="enable_language_switcher"<?php if ($this->options->enable_language_switcher) echo ' checked="checked"'; ?> />
	...
	<?php endif; ?>
		...
		<p>
			<input id="enable_adminbar_switcher" type="checkbox" value="1"
			       name="enable_adminbar_switcher"<?php if ($this->options->enable_adminbar_switcher) echo ' checked="checked"'; ?> />
			<?php _e('extend <em>Backend Admin Bar</em> with a language quick selector.', "wp-native-dashboard"); ?>
		</p>
		<p>
			<input id="translate_front_adminbar" type="checkbox" value="1"
			       name="translate_front_adminbar"<?php if ($this->options->translate_front_adminbar) echo ' checked="checked"'; ?> />
			<?php _e('translate <em>Frontend Admin Bar</em> using backend selected language.', "wp-native-dashboard"); ?>
		</p>
		<p>
			<input id="enable_locale_abbreviations" type="checkbox" value="1"
			       name="enable_locale_abbreviations"<?php if ($this->options->enable_locale_abbreviations) echo ' checked="checked"'; ?> />
		...
		<table id="table_local_i18n" class="widefat fixed" cellspacing="0">
			...
			foreach ($installed as $lang) {
				...
				<tr id="tr-i18n-installed-<?php echo $lang; ?>" class="<?php if ($state) echo 'alternate'; ?>">
					<td><span
							class="i18n-file csp-<?php echo $lang; ?>"><?php echo wp_native_dashboard_get_name_of($lang); ?></span>
					</td>
					...
					<td><?php if ($lang != 'en_US') : ?><a class="csp-delete-local-file"
					                                       href="<?php echo $mo; ?>"><?php _e('Delete', 'wp-native-dashboard'); ?></a>&nbsp;
							<span><img src="images/loading.gif" class="ajax-feedback" title=""
							           alt=""/></span><?php endif; ?></td>
				</tr>
			<?php
			}
			?>
		...
		<p class="csp-read-more center">
			...
			if (count($versions) > 0) {
				...
				foreach ($versions as $version) {
					...
					selected($wp_version, $version);
					echo ">$version</option>";
				}
				echo "</select>";
			}
			?> <a id="csp-check-repository"
			      href="#svn"><?php _e('check repository &raquo;', 'wp-native-dashboard'); ?></a> <span><img
					src="images/loading.gif" class="ajax-feedback" title="" alt=""/></span></p>
		<div id="svn-downloads">
			<div class="progressbar" style="display:none;">
				<div class="widget"
				     style="height:12px; border:1px solid #DDDDDD; background-color:#F9F9F9;width:100%; margin: 3px 0;">
					<div class="widget"
					     style="width: <?php echo min($perc, 100.0) ?>%;height:100%;background-color:<?php echo $color; ?>!important;background-image:none;border-width:0px;text-shadow:0 1px 0 #000000;color:#FFFFFF;text-align:right;font-weight:bold;font-size:8px;margin-bottom:4px;">
						<div
							style="padding:0 10px 0 0; white-space:nowrap;word-wrap:normal!important;overflow: hidden;"><?php echo $perc; ?>
							&nbsp;%
						</div>
					</div>
				</div>
						...
						<div id="post-body-content" class="has-sidebar-content">
							...
							<p class="csp-read-more">
								...
								<label for="cleanup_on_deactivate" class="alignleft">
									<input id="cleanup_on_deactivate" type="checkbox" value="1"
									       name="cleanup_on_deactivate"<?php if ($this->options->cleanup_on_deactivate) echo ' checked="checked"'; ?> />
									<span
										class="csp-warning"><?php _e('cleanup all settings at plugin deactivation.', 'wp-native-dashboard'); ?></span>
								</label>
								<br class="clear"/>
							</p>
