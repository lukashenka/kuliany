...
class wp_native_dashboard_langswitcher
{
	...
	function bp_adminbar_switcher_menu()
	{
		...
		$loc = get_locale();
		?>
		<li id="wp-admin-bar-wpnd-lang-cur" class="csp-langoption"><a href="#"><span><span
				class="csp-<?php echo $loc; ?>"><?php echo wp_native_dashboard_get_name_of($loc); ?></span></span></a>
		<?php
		if (count($langs) > 1) {
			echo '<ul>';
			foreach ($langs as $lang) {
				...
				?>
				<li id="wp-admin-bar-wpnd-lang-<?php echo $lang; ?>" class="csp-langoption csp-langoption-adminbar"><a
						href="#"><span class="csp-<?php echo $lang; ?>"
				                       hreflang="<?php echo $lang; ?>"><?php echo wp_native_dashboard_get_name_of($lang); ?></span></a>
				</li>
			<?php
			}
			echo '</ul>';
	...
	function on_print_admin_bar_switcher()
	{
		...
		$loc = get_locale();
		?>
		<li class="menupop csp-langoption" id="wp-admin-bar-wpnd-lang-cur"><a href="#" aria-haspopup="true"
		                                                                      tabindex="10" class="ab-item"><span
					class="csp-<?php echo $loc; ?>"><?php echo wp_native_dashboard_get_name_of($loc); ?></span></a>

			<div class="ab-sub-wrapper">
				<ul class="ab-submenu" id="wp-admin-bar-wpnd-lang-cur-default">
					<?php foreach ($langs as $lang) {
						if ($lang == $loc) continue;
						?>
						<li class=" csp-langoption csp-langoption-adminbar"
						    id="wp-admin-bar-wpnd-lang-<?php echo $lang; ?>"><a href="#" tabindex="10"
						                                                        class="ab-item"><span
									hreflang="<?php echo $lang; ?>"
									class="csp-<?php echo $lang; ?>"><?php echo wp_native_dashboard_get_name_of($lang); ?></span></a>
						</li>
					<?php } ?>
				</ul>
			</div>
	...
	function on_admin_head()
	{
		...
		<script type="text/javascript">
			...
			function csl_extend_dashboard_header(html) {
				...
				jQuery('#csp-langswitcher-toggle, #csp-langoptions').bind('mouseenter', function () {
					jQuery('#csp-langoptions').removeClass('slideUp').addClass('slideDown');
					setTimeout(function () {
						if (jQuery('#csp-langoptions').hasClass('slideDown')) {
							jQuery('#csp-langoptions').slideDown(100);
							jQuery('#csp-langswitcher-current').addClass('slide-down');
						}
					}, 200)
				});
				jQuery('#csp-langswitcher-toggle, #csp-langoptions').bind('mouseleave', function () {
					jQuery('#csp-langoptions').removeClass('slideDown').addClass('slideUp');
					setTimeout(function () {
						if (jQuery('#csp-langoptions').hasClass('slideUp')) {
							jQuery('#csp-langoptions').slideUp(100, function () {
								jQuery('#csp-langswitcher-current').removeClass('slide-down');
							});
						}
					}, 300)
				});
