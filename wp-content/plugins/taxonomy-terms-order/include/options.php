		...
		<table class="form-table">
			...
			<tr valign="top">
				<th scope="row" style="text-align: right;">
					<label><?php _e("Minimum Level to use this plugin", 'to') ?></label></th>
				<td>
					<select id="role" name="level">
						<option
							value="0" <?php if ($options['level'] == "0") echo 'selected="selected"' ?>><?php _e('Subscriber', 'tto') ?></option>
						<option
							value="1" <?php if ($options['level'] == "1") echo 'selected="selected"' ?>><?php _e('Contributor', 'tto') ?></option>
						<option
							value="2" <?php if ($options['level'] == "2") echo 'selected="selected"' ?>><?php _e('Author', 'tto') ?></option>
						<option
							value="5" <?php if ($options['level'] == "5") echo 'selected="selected"' ?>><?php _e('Editor', 'tto') ?></option>
						<option
							value="8" <?php if ($options['level'] == "8") echo 'selected="selected"' ?>><?php _e('Administrator', 'tto') ?></option>
					</select>
				</td>
			</tr>
			...
			<tr valign="top">
				...
				<td>
					<select id="role" name="autosort">
						<option
							value="0" <?php if ($options['autosort'] == "0") echo 'selected="selected"' ?>><?php _e('OFF', 'tto') ?></option>
						<option
							value="1" <?php if ($options['autosort'] == "1") echo 'selected="selected"' ?>><?php _e('ON', 'tto') ?></option>
			...
			<tr valign="top">
				...
				<td>
					<label for="users_can_register">
						<input type="checkbox" <?php if ($options['adminsort'] == "1") {
			...
			<tr valign="top">
				...
				<td>
					...
					<p class="example"><?php _e('No query will be changed, the terms will appear in the original order. To retrieve the terms in the required order you must use the term_order on the orderby parameter', 'to') ?>
						:</p>
                                <pre class="example">
$argv = array(
                'orderby'       =>  'term_order',
...
get_terms('category', $argv);
					...
					<p><?php _e("See more info on the get_terms usage", 'to') ?> <a
							href="http://codex.wordpress.org/Function_Reference/get_terms"
							target="_blank"><?php _e("here", 'to') ?></a></p>
