...
function wppb_general_settings()
{
	...
	<form method="post" action="options.php#general-settings">
		...
		<select name="wppb_general_settings[extraFieldsLayout]" class="wppb_general_settings">
			<option
				value="yes" <?php if ($wppb_generalSettings['extraFieldsLayout'] == 'yes') echo 'selected'; ?>><?php _e('Default', 'profilebuilder'); ?></option>
			<?php
			if (file_exists($wppb_premiumStyle . 'custom.fields.php')) {
				?>
				<option
					value="white" <?php if ($wppb_generalSettings['extraFieldsLayout'] == 'white') echo 'selected'; ?>><?php _e('White', 'profilebuilder'); ?></option>
				<option
					value="black" <?php if ($wppb_generalSettings['extraFieldsLayout'] == 'black') echo 'selected'; ?>><?php _e('Black', 'profilebuilder'); ?></option>
			<?php
			}
			?>
			<option
				value="no" <?php if ($wppb_generalSettings['extraFieldsLayout'] == 'no') echo 'selected'; ?>><?php _e('None', 'profilebuilder'); ?></option>
		</select>
		...
		<div id="general_settings_registration_section">
			...
			if (!is_multisite()) {
				?>
				<font
					id="generalSettingFont"><?php _e('"Email Confirmation" Feature Activated:', 'profilebuilder'); ?></font>
				<select name="wppb_general_settings[emailConfirmation]" class="wppb_general_settings2"
				        onchange="wppb_display_page_select(this.value)">
					<option
						value="yes" <?php if ($wppb_generalSettings['emailConfirmation'] == 'yes') echo 'selected'; ?>><?php _e('Yes', 'profilebuilder'); ?></option>
					<option
						value="no" <?php if ($wppb_generalSettings['emailConfirmation'] == 'no') echo 'selected'; ?>><?php _e('No', 'profilebuilder'); ?></option>
				</select>
				<br/><br/>
			<?php
			} else {
			...
			<div id="general_settings_registration_page_div">
				...
				<select name="wppb_general_settings[activationLandingPage]" class="wppb_general_settings5">
					<option
						value="" <?php if ($wppb_generalSettings['emailConfirmation'] == '') echo 'selected'; ?>></option>
		...
		if (file_exists($wppb_premiumStyle . 'custom.fields.php')) {
			...
			<select id="adminApprovalSelect" name="wppb_general_settings[adminApproval]" class="wppb_general_settings3">
				<option
					value="yes" <?php if ($wppb_generalSettings['adminApproval'] == 'yes') echo 'selected'; ?>><?php _e('Yes', 'profilebuilder'); ?></option>
				<option
					value="no" <?php if ($wppb_generalSettings['adminApproval'] == 'no') echo 'selected'; ?>><?php _e('No', 'profilebuilder'); ?></option>
			</select>
		...
		<select name="wppb_general_settings[loginWith]" class="wppb_general_settings4">
			<option
				value="username" <?php if ($wppb_generalSettings['loginWith'] == 'username') echo 'selected'; ?>><?php _e('Username', 'profilebuilder'); ?></option>
			<option
				value="email" <?php if ($wppb_generalSettings['loginWith'] == 'email') echo 'selected'; ?>><?php _e('Email', 'profilebuilder'); ?></option>
		</select>
		<?php
		if (file_exists($wppb_premiumStyle . 'custom.fields.php'))
			echo '</div>';
