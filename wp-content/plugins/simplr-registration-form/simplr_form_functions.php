...
function simplr_validate($data, $atts)
{
	...
	if ('yes' == @$atts['password']) {
		if ($data['password'] != $data['password_confirm']) {
			$errors[] = "Паролі не супадаюць";
			add_filter('password_error_class', '_sreg_return_error');
		} elseif (empty($data['password'])) {
			$errors[] = "Вы павінны пазначыць пароль";
			add_filter('password_error_class', '_sreg_return_error');
		}
	}
	...
	// Validate username
	if (!$data['username']) {
		$errors[] = "Вы павінны пазначыць лагін";
		...
		if ($user_test != true) {
			$errors[] .= "Няправільны лагін";
			add_filter('username_error_class', '_sreg_return_error');
		}
		// check whether username already exists
		$user_id = username_exists($data['username']);
		if ($user_id) {
			$errors[] .= "Такі кулянец ужо існуе";
			add_filter('username_error_class', '_sreg_return_error');
		}
...
function sreg_process_form($atts)
{
	...
	if (!wp_verify_nonce($_POST['simplr_nonce'], 'simplr_nonce')) {
		die('Security check');
	}
...
function simplr_setup_user($atts, $data)
{
	...
	if ('administrator' == $role) {
		wp_die('Do not use this form to register administrators');
	}
	require_once(ABSPATH . WPINC . '/registration.php');
	require_once(ABSPATH . WPINC . '/pluggable.php');
	...
	foreach ($page as $node) {
		...
		if ($error->length>0) {
			$error = $error->item(0)->getAttribute("reason");
